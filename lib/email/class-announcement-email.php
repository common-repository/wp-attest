...
if ( ! class_exists( 'ATTEST_LMS_ANNOUNCEMENT_EMAIL' ) ) {

	final class ATTEST_LMS_ANNOUNCEMENT_EMAIL {
    ...
    public function execute() {
			...
			$sent = false;
			$this->permission = get_option('attest_email_announcement_permission');
			$this->subject = get_option('attest_email_announcement_subject');
			$this->body = get_option('attest_email_announcement_body');

			if ($this->permission != '1') {
				return;
			}

      $students = $this->get_students($this->course_id);
      if ($students) {
    ...
    public function get_email($student_id) {
      ...
      return $email;
		...
		public function alter_text($text, $user_id) {
			...
			$text = str_replace( '{announcement_title}', $this->title, $text);
			$text = str_replace( '{announcement_description}', $this->description, $text);
